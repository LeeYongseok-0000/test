...
export const SearchProvider = ({ children }) => {
  ...
  const getImageUrl = (uploadFileNames) => {
    if (uploadFileNames && uploadFileNames.length > 0) {
      // 배열의 첫 번째 파일 이름을 사용합니다.
      const encodedFileName = encodeURIComponent(uploadFileNames[0]);
  ...
  useEffect(() => {
    ...
    productsApi
      ...
      .then((res) => {
        const mappedProducts = res.dtoList.map((item) => ({
          ...item,
          imageUrl: getImageUrl(item.uploadFileNames), // 여기서 imageUrl을 생성합니다.
        }));
        setInitialProducts(mappedProducts);
      })
      .catch((err) => {
  ...
  const performTextSearch = useCallback(
    async (query) => {
      ...
      try {
        ...
        const mappedResults = response.dtoList.map((item) => ({
          ...item,
          // 백엔드에서 받은 uploadFileNames를 사용해서 imageUrl을 생성합니다.
          imageUrl: getImageUrl(item.uploadFileNames),
        }));
