...
public class JWTCheckFilter extends OncePerRequestFilter {
  ...
  protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
    ...
    if (path.startsWith("/api/donations")) return true;
  ...
  protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
    ...
    try {
      ...
      try {
        ...
        log.error("Token validation failed: {}", tokenException.getMessage());
        
        // 토큰이 만료되었거나 유효하지 않은 경우에도 임시로 통과시킴 (테스트용)
        log.warn("Bypassing token validation for testing purposes");
        claims = Map.of(
          "email", "12@aaa.com", // 실제 로그인한 사용자 이메일로 변경
          "pw", "",
          "nickname", "테스트",
          "social", false,
